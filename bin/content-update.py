...
def dump(obj):
    for attr in dir(obj):
        print("obj.{0} = {0}".format((attr, getattr(obj, attr))))
...
def send_message(version, splunk_host,splunk_user,splunk_password):
    ...
    splunkService.post('/services/messages', name="Splunk_Analytic_Story_Execution", value="ESCU was successfully updated to {0}".format(version), severity="info")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output", required=False, default=os.getcwd(), help="directory where to write the sources files to..example: /opt/splunk/etc/apps/Splunk_Analytic_Story_Execution/default")
    ...
    parser.add_argument("--splunk_app", required=False, default="Splunk_Analytic_Story_Execution", help="the application name to reload, defaults to: Splunk_Analytic_Story_Execution")
